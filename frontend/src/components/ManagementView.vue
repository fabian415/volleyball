...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <!-- 手機版置頂區：開始比賽 + 本輪賽程優先顯示，桌面版維持原排版（隱藏） -->
    <div v-if="state.teamsAssigned" class="lg:hidden space-y-6">
      <button
        v-if="state.currentMatchIndex < 3"
    ...
    <div class="lg:col-span-2 space-y-6">
      ...
      <template v-else>
        ...
        <!-- 賽程：手機版已於上方顯示，這裡僅桌面版顯示，避免重複 -->
        <MatchScheduleCard class="hidden lg:block" />
      </template>
    </div>
    ...
    <div class="flex flex-col gap-4 h-fit sticky top-8">
      <button
        ...
        @click="setView('dashboard')"
        class="hidden lg:flex w-full bg-indigo-600 text-white py-4 rounded-2xl hover:bg-indigo-700 active:scale-95 transition items-center justify-center gap-3 font-black text-lg shadow-lg shadow-indigo-500/30 border-b-4 border-indigo-800"
      >
        <Monitor :size="24" /> 開始比賽
      </button>
